...
var STR = {
  zh: {
    ...
    exporting: "导出中…",
    swInject: "注入会话上下文",
    swInjectD: "每轮注入一段极短说明，让模型知道可主动发起拟合。开启才有 token 开销",
    swMem: "写入记忆插件",
    swMemD: "把收敛结论写进 dsh-auto-memory。默认关闭以避免耦合",
    swTools: "向模型暴露工具",
    swToolsD: "模型可见 memory_fit_* 工具。注册即占每轮 tool 上下文",
    swArch: "本地留档",
    swArchD: "拟合全过程写入 ~/.dsh/memory-fitting/sessions/（隔离于其它插件）",
    swPersist: "记住悬窗开合",
    swPersistD: "关闭则每次启动都收起悬窗",
    secSurface: "污染面开关 · 默认关闭",
    secStore: "本插件自己的存储",
    secParams: "拟合参数",
    secEnv: "环境自检",
    pQ: "每轮问题数上限",
    ...
    pR: "整场轮数上限",
    pRD: '防止"问上瘾"，默认 4 轮封顶',
    amName: "dsh-auto-memory",
    amNone: "未检测到（不影响本地留档）",
    amAlive: "已检测到 · 实例存活",
    amDead: "已检测到 · 实例未响应",
    sbName: "写入安全带",
    sbDesc: "归档前对用户原话做字符级改写，防止锚点击穿记忆文件",
    ok: "通过",
    bad: "异常",
    okShort: "OK",
    none: "—",
    ...
    archiving: "归档中…",
    rename: "改标题",
    renamePrompt: "给这条留档起个可读标题（拟合后回填节点名）",
    del: "删除",
    delQ: "删除这条留档？会话文件与索引项都会被移除，不可恢复。",
    delYes: "确认删除",
    delNo: "取消",
    deleting: "删除中…",
    stAccepted: "已确认",
    ...
    stDismissed: "已放弃",
    fitTitle: "发起一次拟合",
    fitHint: "写下触发语 → 开始 → 到聊天窗口回答模型提出的问题。",
    fitPh: "例如：帮我把记忆这部分弄好一点，感觉不太行。",
    fitStart: "立即开始",
    fitStartD: "在本地建立拟合会话并记入留档",
    fitGo: "开始",
    fitCreating: "创建中…",
    fitViaAi: "让 AI 主动发起",
    ...
    tooltipOpen: "记忆拟合",
    tooltipClose: "关闭面板（留档不受影响）",
    tooltipSwitch: "点击切换",
    posReset: "面板位置已复位",
    dragHint: "拖动标题栏可移动 · 双击复位"
  },
  en: {
    title: "Memory Fitting",
    ...
    export: "Export pairs",
    exporting: "Exporting…",
    swInject: "Inject session context",
    swInjectD: "One short note per turn so the model knows it may start a fitting. Costs tokens only when on.",
    swMem: "Write to memory plugin",
    swMemD: "Write the converged conclusion into dsh-auto-memory. Off by default to avoid coupling.",
    swTools: "Expose tools to the model",
    swToolsD: "The model can see the memory_fit_* tools. Registration costs tool-schema context every turn.",
    swArch: "Local archive",
    swArchD: "Write the whole fitting to ~/.dsh/memory-fitting/sessions/ (isolated from other plugins)",
    swPersist: "Remember panel state",
    swPersistD: "Off means the panel starts collapsed every launch",
    secSurface: "Contamination surface · all off by default",
    secStore: "This plugin's own storage",
    secParams: "Fitting parameters",
    secEnv: "Environment check",
    pQ: "Max questions per round",
    pQD: "How many questions one round may contain",
    pR: "Max rounds",
    pRD: "Guards against asking forever; 4 by default",
    amName: "dsh-auto-memory",
    amNone: "not detected (local archive unaffected)",
    amAlive: "detected · instance alive",
    amDead: "detected · instance not responding",
    sbName: "Write seatbelt",
    sbDesc: "Character-level rewriting of user text before archiving, so the memory file cannot be locked",
    ok: "pass",
    bad: "FAIL",
    okShort: "OK",
    none: "n/a",
    ...
    back: "← Back to list",
    confirmArch: "Confirm and archive",
    archiving: "Archiving…",
    rename: "Rename",
    renamePrompt: "Give this archive a readable title (a work-node name works well)",
    del: "Delete",
    delQ: "Delete this archive? The session file and index entry are removed. Irreversible.",
    delYes: "Confirm delete",
    delNo: "Cancel",
    deleting: "Deleting…",
    stAccepted: "confirmed",
    stProposed: "pending",
    stFitting: "in progress",
    stDismissed: "abandoned",
    fitTitle: "Start a fitting",
    fitHint: "Write a trigger line → Start → answer the questions in the chat window.",
    fitPh: "e.g. Just make that memory part better, it feels off.",
    fitStart: "Start now",
    fitStartD: "Create the fitting session locally and record it in the archive",
    fitGo: "Start",
    fitCreating: "Creating…",
    ...
    evCount: "Event stream · ",
    evFrames: " frames",
    evEmpty: "(empty)",
    needUtterance: "Write a trigger line first",
    createFail: "Create failed",
    netErr: "network error",
    saveFail: "Failed to save settings",
    copied: " preference pairs copied to clipboard",
    clipboardNo: "Clipboard unavailable",
    noPairs: "No exportable pairs yet (answer some questions first)",
    tooltipOpen: "Memory Fitting",
    tooltipClose: "Close panel (archiving is unaffected)",
    tooltipSwitch: "Click to toggle",
    posReset: "Panel position reset",
    dragHint: "Drag the title bar to move · double-click to reset"
  }
};
function detectLang() {
  try {
...
function renderArchive(body) {
  ...
  exp.onclick = function() {
    ...
    req(API.export).then(function(r) {
      ...
      if (!r.count) {
        toast("还没有可导出的偏好对（需要先答过题）", true);
        return;
      }
      var done = function() {
        toast("已复制 " + r.count + " 条偏好对到剪贴板");
      };
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(r.jsonl).then(done).catch(function() {
          toast("剪贴板不可用", true);
        });
      } else {
        toast("剪贴板不可用", true);
